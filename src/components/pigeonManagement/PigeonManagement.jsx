...
import EditPigeon from "./EditPigeon";
...
import { FaTrash, FaEye, FaEdit } from "react-icons/fa"; // ✅ icons from react-icons
import { useGetAllPigeonsQuery } from "../../redux/apiSlices/allpigeonSlice";
...
const PigeonManagement = () => {
  ...
  const handleModalSave = async (values) => {
    await updatePigeon({ id: editingPigeon._id, data: values });
    refetch();
    setIsModalVisible(false);
    setEditingPigeon(null);
  };

  const handleDelete = async (pigeon) => {
    Swal.fire({
      title: "Are you sure?",
  ...
  const filteredData = useMemo(() => {
    if (!pigeonResponse?.data?.data) return [];
    return pigeonResponse.data.data.filter((item) => {
      const matchesSearch =
  ...
  // rest of Table and UI code remains the same
  const getColumns = () => [
    {
      title: "Image",
      dataIndex: "image",
        ...
        <div className="overflow-x-auto border rounded-lg shadow-md bg-gray-50 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
          <div className="border rounded-lg shadow-md bg-gray-50">
            <div
              style={{
                minWidth:
                  pigeonResponse?.data?.data?.length > 0
                    ? "max-content"
                    : "100%",
              }}
            ...
            >
              {isLoading ? (
                ...
                <Table
                  columns={getColumns()}
                  dataSource={pigeonResponse?.data?.data || []}
                  ...
                  pagination={{
                    total: pigeonResponse?.data?.pagination?.total || 0,
                    pageSize: pigeonResponse?.data?.pagination?.limit || 10,
                    current: pigeonResponse?.data?.pagination?.page || 1,
